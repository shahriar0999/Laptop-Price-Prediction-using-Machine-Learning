...
class OSStandardiser(BaseEstimator, TransformerMixin):
    OS_MAP = {'macos':'macOS','mac':'macOS','windows':'Windows',
              'linux':'Linux','chrome':'Chrome OS','no':'No OS'}
    ...
    def transform(self, X, y=None):
        ...
        for col in X.columns:
            X[col] = (X[col].astype(str).str.lower().str.strip()
                      .map(self.OS_MAP).fillna('Other'))
        return X.values

    def get_feature_names_out(self, input_features=None):
        if input_features is not None: return np.array(input_features)
        return np.array(['os'])
...
def log_candidate(params, stage, rank, parent_run_id):
    ...
    """
    run_name = f"{stage} | rank#{rank:02d} | " \
...
def build_fine_grid(best_params):
    ...
    return {
        ...
        "model__learning_rate": sorted(set([
            round(max(0.005, lr-0.02), 3),
            round(lr, 3),
            round(lr+0.02, 3)
        ])),
        "model__max_depth":     sorted(set([max(2, d-1), d, d+1])),
        "model__subsample":     sorted(set([
...
print(f"  Best Stage A R²      : {best_a_r2:.4f}")
print(f"  Best Stage B R²      : {best_b_r2:.4f}")
print(f"  Overall Best R²      : {overall_best['r2_log']:.4f}  "
      f"(Δ {overall_best['r2_log']-baseline_r2:+.4f})")
print(f"  Overall Best Params  : {overall_best['params']}")

print("\n\n── Top 10 Candidates (all stages) ──────────────────────────")
top10 = all_df.head(10)[['stage','rank','r2_log','rmse_log',
                           'r2_orig','rmse_orig','train_time','params']]
