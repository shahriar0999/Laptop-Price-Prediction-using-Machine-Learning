...
print(
    f"  Target — Price  min={df['Price'].min():.0f} "
...
print("  Target  : log2(Price) — stabilises variance")
...
MODELS = {
    ...
    "Decision Tree": {
        "model": DecisionTreeRegressor(
            max_depth=10, min_samples_split=10, min_samples_leaf=5, random_state=33
        ),
    ...
    "Extra Trees": {
        "model": ExtraTreesRegressor(
            n_estimators=200, min_samples_leaf=2, random_state=33, n_jobs=-1
        ),
    ...
    "AdaBoost": {
        "model": AdaBoostRegressor(
            n_estimators=200, learning_rate=0.05, random_state=33
        ),
...
with mlflow.start_run(run_name="All Models Comparison — Parent") as parent_run:
    ...
    for model_name, cfg in MODELS.items():
        ...
        with mlflow.start_run(
            ...
            print(
                f"    R²(log)={r2_log:.4f}  RMSE(log)={rmse_log:.4f}  "
